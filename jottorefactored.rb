...
def one_player
	...
	computer_word = Word.new(random_word(word_sort(compwordlist)))
...
def turn(computer_word, guess_list, play_list)
  ...
  if guess == computer_word
    ...
    rematch
  	...
  	puts "5!"
  	puts "You've got all the letters, but not in the right order."
...
class CompareWords
	def initialize(chosen_word, guessable_word)
		@chosen_word = chosen_word
		@guessable_word = guessable_word
	end

	def compare
		letters.inject(0) do |sum, value| 
			if 
			sum += 1 if word2.downcase.split("").include?(letter)
		end
	end
...
class Word
	...
	def compare(other_word)
		letters.inject(0) do |sum, letter| 
			if other_word.letters.include?(letter)
			  sum += 1
		    end
		end
	end
